...
class Graphics3D {
	...
	public void addVertex(float x, float y, float z) {
		...
		vertices[primitivePos].set(x, y, z, 1.0f);
		primitivePos++;
	...
	private void transform(int start, int num) {
		...
		for (int i=start; i<num; i++) {
			...
			if (lightingEnabled && primitiveType != LINES) {
				...
				clamp(colors[i], 0.0f, 1.0f);
			}
		}
	}
	
	private static void clamp(Tuple3f t, float min, float max) {
	...
	private Boolean clip(int num) {
		...
		for (int i=0; i<clippedCount; i++) {
			clippedVertices[i].scale(1.0f / clippedVertices[i].w);
			
			clippedVertices[i].x = (clippedVertices[i].x+ 1.0f) * rasterizer.getWidth() * 0.5f;
			clippedVertices[i].y = rasterizer.getHeight() - 1 - (clippedVertices[i].y+ 1.0f) * rasterizer.getHeight() * 0.5f;
		}
