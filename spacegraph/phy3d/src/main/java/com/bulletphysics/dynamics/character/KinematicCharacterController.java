...
public class KinematicCharacterController extends ActionInterface {
    ...
    private static final Vector3f[] upAxisDirection = {
    ...
    private void playerStep(CollisionWorld collisionWorld, float dt) {
        ...
        if (useWalkDirection) {
            ...
            float dtMoving = Math.min(dt, velocityTimeInterval);
    ...
    private boolean recoverFromPenetration(CollisionWorld collisionWorld) {
        ...
        collisionWorld.dispatcher().dispatchAllCollisionPairs(
                ghostObject.getOverlappingPairCache(), collisionWorld.getDispatchInfo(), collisionWorld.dispatcher());
    ...
    private void stepUp(CollisionWorld world) {
        ...
        up.scale(-1.0f, upAxisDirection[upAxis]);
