...

        {
...

            {
                ...
                if (len > 0.00001f) {
                    ...
                    velrelOrthog.scale((1.0f / denom) * relaxationFactor);
                }
                ...
                if (solveLimit) {
                    ...
                    float amplitude = ((tmp).dot(axisA) * relaxationFactor + correction * (1.0f / timeStep) * biasFactor) * limitSign;

                    float impulseMag = amplitude * kHinge;

                    // Clamp the accumulated impulse
                    float temp = accLimitImpulse;
                    accLimitImpulse = Math.max(accLimitImpulse + impulseMag, 0.0f);
