...
public class Generic6DofConstraint extends TypedConstraint {
    ...
    private final JacobianEntry[] jacLinear/*[3]*/ = {new JacobianEntry(), new JacobianEntry(), new JacobianEntry()}; //!< 3 orthogonal linear constraints
    private final JacobianEntry[] jacAng/*[3]*/ = {new JacobianEntry(), new JacobianEntry(), new JacobianEntry()}; //!< 3 orthogonal angular constraints
    ...
    private final Vector3f[] calculatedAxis/*[3]*/ = {new Vector3f(), new Vector3f(), new Vector3f()};
    ...
    public void buildJacobian() {
        ...
        linearLimits.accumulatedImpulse.set(0.0f, 0.0f, 0.0f);
        for (int i = 0; i < 3; i++) {
            angularLimits[i].accumulatedImpulse = 0.0f;
        }
    ...
    public void solveConstraint(float timeStep) {
        ...
        for (i = 0; i < 3; i++) {
            if (linearLimits.isLimited(i)) {
                jacDiagABInv = 1.0f / jacLinear[i].getDiagonal();
        ...
        for (i = 0; i < 3; i++) {
            if (angularLimits[i].needApplyTorques()) {
                ...
                angularJacDiagABInv = 1.0f / jacAng[i].getDiagonal();
