...
public class ConeTwistConstraint extends TypedConstraint {
    ...
    public void buildJacobian() {
        ...
        swingCorrection = 0.0f;
        twistLimitSign = 0.0f;
        solveTwistLimit = false;
        solveSwingLimit = false;
        accTwistLimitImpulse = 0.0f;
        accSwingLimitImpulse = 0.0f;
        ...
        if (!angularOnly) {
            ...
            // TODO: stack
            Vector3f[] normal/*[3]*/ = {new Vector3f(), new Vector3f(), new Vector3f()};
...
//			swing2 = ScalarUtil.atan2Fast(b2Axis1.dot(b1Axis3), b2Axis1.dot(b1Axis1));
        ...
        if (EllipseAngle > 1.0f) {
            ...
            kSwing = 1.0f / (getRigidBodyA().computeAngularImpulseDenominator(swingAxis) +
        ...
        if (twistSpan >= 0.0f) {
            ...
            float lockedFreeFactor = (twistSpan > 0.05f) ? limitSoftness : 0.0f;
            if (twist <= -twistSpan * lockedFreeFactor) {
                ...
                kTwist = 1.0f / (getRigidBodyA().computeAngularImpulseDenominator(twistAxis) +
    ...
    public void solveConstraint(float timeStep) {
        ...
        if (!angularOnly) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                float jacDiagABInv = 1.0f / jac[i].getDiagonal();
...

        {
            ...
            if (solveSwingLimit) {
                ...
                float amplitude = ((tmp).dot(swingAxis) * relaxationFactor * relaxationFactor + swingCorrection * (1.0f / timeStep) * biasFactor);
            ...
            if (solveTwistLimit) {
                tmp.sub(angVelB, angVelA);
                float amplitude = ((tmp).dot(twistAxis) * relaxationFactor * relaxationFactor + twistCorrection * (1.0f / timeStep) * biasFactor);
