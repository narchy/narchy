...
public class SliderConstraint extends TypedConstraint {
    ...
    private void initParams() {
        ...
        targetLinMotorVelocity = 0.0f;
        maxLinMotorForce = 0.0f;
        accumulatedLinMotorImpulse = 0.0f;

        poweredAngMotor = false;
        targetAngMotorVelocity = 0.0f;
        maxAngMotorForce = 0.0f;
        accumulatedAngMotorImpulse = 0.0f;
    }
    ...
    private void buildJacobianInt(RigidBody rbA, RigidBody rbB, Transform frameInA, Transform frameInB) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            jacLin[i].init(
                    ...
                    rbB.getInvMass());
            jacLinDiagABInv[i] = 1.0f / jacLin[i].getDiagonal();
        ...
        kAngle = 1.0f / (rbA.computeAngularImpulseDenominator(axisA) + rbB.computeAngularImpulseDenominator(axisA));
    ...
    private void solveConstraintInt(RigidBody rbA, RigidBody rbB) {
        ...
        if (len > 0.00001f) {
            ...
            velrelOrthog.scale((1.0f / denom) * dampingOrthoAng * softnessOrthoAng);
        }
        ...
        if (len2 > 0.00001f) {
            ...
            angularError.scale((1.0f / denom2) * restitutionOrthoAng * softnessOrthoAng);
        }
