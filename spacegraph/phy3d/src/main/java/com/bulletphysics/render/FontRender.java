...
public class FontRender {
	...
	private static BufferedImage renderFont(Font font, boolean antialiasing, Glyph[] glyphs) {
        ...
        img = switch (y) {
            case int i when i < 128 -> img.getSubimage(0, 0, img.getWidth(), 128);
            case int i when i < 256 -> img.getSubimage(0, 0, img.getWidth(), 256);
            case int i when i < 512 -> img.getSubimage(0, 0, img.getWidth(), 512);
            default -> img;
        };
