...
public abstract class TriangleMeshShape extends ConcaveShape {
	...
	final Vector3f localAabbMin = new Vector3f();
	final Vector3f localAabbMax = new Vector3f();
	...
	private Vector3f localGetSupportingVertex(Vector3f vec, Vector3f out) {
	...
	public void recalcLocalAabb() {
		for (int i = 0; i < 3; i++) {
			...
			VectorUtil.coord(vec, i, 1.0f);
			Vector3f tmp = localGetSupportingVertex(vec, new Vector3f());
			VectorUtil.coord(localAabbMax, i, VectorUtil.coord(tmp, i) + collisionMargin);
			VectorUtil.coord(vec, i, -1.0f);
			localGetSupportingVertex(vec, tmp);
			VectorUtil.coord(localAabbMin, i, VectorUtil.coord(tmp, i) - collisionMargin);
		}
	}
	...
	private static class SupportVertexCallback extends TriangleCallback {
		...
		SupportVertexCallback(Vector3f supportVecWorld, Transform trans) {
		...
		Vector3f getSupportVertexLocal(Vector3f out) {
			out.set(supportVertexLocal);
			return out;
		}
	}
	...
	private static class FilteredCallback extends InternalTriangleIndexCallback {
		...
		FilteredCallback(TriangleCallback callback, Vector3f aabbMin, Vector3f aabbMax) {
