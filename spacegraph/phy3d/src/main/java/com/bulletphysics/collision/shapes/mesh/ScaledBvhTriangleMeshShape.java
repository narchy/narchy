...
public class ScaledBvhTriangleMeshShape extends ConcaveShape {
	...
	public void processAllTriangles(TriangleCallback callback, Vector3f aabbMin, Vector3f aabbMax) {
		...
		scaledAabbMin.x = (localScaling.x >= 0.0f ? aabbMin : aabbMax).x * invLocalScaling.x;
		scaledAabbMin.y = (localScaling.y >= 0.0f ? aabbMin : aabbMax).y * invLocalScaling.y;
		scaledAabbMin.z = (localScaling.z >= 0.0f ? aabbMin : aabbMax).z * invLocalScaling.z;

		scaledAabbMax.x = (localScaling.x <= 0.0f ? aabbMin : aabbMax).x * invLocalScaling.x;
		scaledAabbMax.y = (localScaling.y <= 0.0f ? aabbMin : aabbMax).y * invLocalScaling.y;
		scaledAabbMax.z = (localScaling.z <= 0.0f ? aabbMin : aabbMax).z * invLocalScaling.z;
	...
	public void getAabb(Transform trans, Vector3f aabbMin, Vector3f aabbMax) {
		...
		localAabbMin.x = (localScaling.x >= 0.0f ? tmpLocalAabbMin : tmpLocalAabbMax).x;
		localAabbMin.y = (localScaling.y >= 0.0f ? tmpLocalAabbMin : tmpLocalAabbMax).y;
		localAabbMin.z = (localScaling.z >= 0.0f ? tmpLocalAabbMin : tmpLocalAabbMax).z;
		localAabbMax.x = (localScaling.x <= 0.0f ? tmpLocalAabbMin : tmpLocalAabbMax).x;
		localAabbMax.y = (localScaling.y <= 0.0f ? tmpLocalAabbMin : tmpLocalAabbMax).y;
		localAabbMax.z = (localScaling.z <= 0.0f ? tmpLocalAabbMin : tmpLocalAabbMax).z;
