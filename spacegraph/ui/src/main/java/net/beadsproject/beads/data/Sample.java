...
import net.beadsproject.beads.file.AudioFileReader;
import net.beadsproject.beads.file.AudioFileType;
import net.beadsproject.beads.file.AudioFileWriter;
...
public class Sample {
    ...
    static {
        try {
            ...
            try {
                defaultAudioFileReaderClass = (Class<? extends AudioFileReader>) Class.forName("net.beadsproject.beads.file.WavFileReaderWriter");
    ...
    static {
        try {
            ...
            try {
                defaultAudioFileWriterClass = (Class<? extends AudioFileWriter>) Class.forName("net.beadsproject.beads.file.WavFileReaderWriter");
            } catch (ClassNotFoundException e2) {
                defaultAudioFileReaderClass = null;
            }
        }
    ...
    public void clear() {
        for (int i = 0; i < nChannels; i++) {
            Arrays.fill(theSampleData[i], 0.0f);
        }
    ...
    private void write(String fn, AudioFileType type, SampleAudioFormat saf) throws IOException {
        ...
        if (type == AudioFileType.WAV) {
            try {
                theRealAudioFileWriterClass = (Class<? extends AudioFileWriter>) Class.forName("net.beadsproject.beads.file.WavFileReaderWriter");
            } catch (ClassNotFoundException e) {
    ...
    public double getLength() {
        return 1000.0f * this.nFrames / this.sampleRate;
    }
    ...
    private void loadAudioFile(String file) throws IOException {
        ...
        if (file.endsWith(".wav") || file.endsWith(".WAV")) {
            try {
                theRealAudioFileReaderClass = (Class<? extends AudioFileReader>) Class.forName("net.beadsproject.beads.file.WavFileReaderWriter");
            } catch (ClassNotFoundException e) {
