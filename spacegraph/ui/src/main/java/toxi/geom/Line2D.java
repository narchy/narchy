...
public class Line2D {
    ...
    public LineIntersection intersectLine(Line2D l) {
        ...
        if (denom != 0.0) {
			...
			isec = new LineIntersection(ua >= 0.0f && ua <= 1.0 && ub >= 0.0 && ub <= 1.0 ? Type.INTERSECTING : Type.NON_INTERSECTING, i, ua, ub);
        } else {
            if (na == 0.0 && nb == 0.0) {
				isec = new LineIntersection(distanceToPoint(l.a) == 0.0 ? Type.COINCIDENT : Type.COINCIDENT_NO_INTERSECT, null);
            } else {
