...
public class StreamXUtil {
    ...
    public static class LayerNormLinear implements UnaryOperator<Tensor> {
        ...
        public LayerNormLinear(int in, int out, float sparsity, float sparseNoise) {
     ...
     * Online observation normalization using Welford's algorithm
     */
    public static class ObservationNormalizerRollingWelford {
        final FloatMeanWindow[] mean;

        private transient final Tensor MEAN, VAR;

        public ObservationNormalizerRollingWelford(int dim, int history) {
    ...
    public static class ObGD implements OptimizerStep {
...
//        private static final boolean clip = false;
        ...
        public EligibilityTraces(FloatSupplier gamma, FloatSupplier lambda) {
            this.gamma = gamma;
            this.lambda = lambda;
            reset();
        }
