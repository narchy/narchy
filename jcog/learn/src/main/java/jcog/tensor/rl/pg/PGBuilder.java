 ...
 */
record Experience(Tensor state, double[] action, double reward, Tensor nextState, boolean done,
...
public class PGBuilder {
    ...
    private static class SACConfigurator implements AlgorithmConfigurator {
        ...
        public AlgorithmStrategy buildStrategy(PGBuilder builder) {
            ...
            var buffer = new ReplayBuffer(m.replayBuffer().capacity());
...
class PolicyGradientModel extends AbstrPG {
    ...
    public double[] act(double[] input, double reward, boolean done) {
        ...
        if (lastState != null) {
            strategy.record(new Experience(lastState, lastAction, reward, currentState, done, lastLogProb));
        }
...
class ReplayBuffer implements Memory {
...
class ReinforceStrategy extends AbstractStrategy {
    ...
    @Override
    public void record(Experience e) {
        memory.add(e);
    ...
    public void update(long totalSteps) {
        ...
        updateSteps++;
        var states = Tensor.concatRows(episode.stream().map(Experience::state).toList());
    ...
    private Tensor computeReturns(List<Experience> episode) {
        var n = episode.size();
...
class PPOStrategy extends AbstractStrategy {
    ...
    public void update(long totalSteps) {
        ...
        var states = Tensor.concatRows(episode.stream().map(Experience::state).toList());
        var actions = Tensor.concatRows(episode.stream().map(e -> Tensor.row(e.action())).toList());
        var oldLogProbs = Tensor.concatRows(episode.stream().map(Experience::oldLogProb).filter(Objects::nonNull).toList());
    ...
    private Tensor[] computeGAE(List<Experience> episode, Tensor values) {
        var n = episode.size();
...
abstract class OffPolicyStrategy extends AbstractStrategy {
    ...
    protected final ReplayBuffer memory;

    protected OffPolicyStrategy(MemoryConfig m, ReplayBuffer memory) {
        this.m = m;
        this.memory = memory;
    }

    protected static Batch toBatch(Collection<Experience> experiences) {
        return new Batch(
                Tensor.concatRows(experiences.stream().map(Experience::state).toList()),
                Tensor.concatRows(experiences.stream().map(e -> Tensor.row(e.action())).toList()),
                Tensor.row(experiences.stream().mapToDouble(Experience::reward).toArray()).transpose(),
                Tensor.concatRows(experiences.stream().map(Experience::nextState).toList()),
...
class SACStrategy extends OffPolicyStrategy {
    ...
    public SACStrategy(HyperparamConfig h, ActionConfig a, MemoryConfig m, ReplayBuffer memory, GaussianPolicy policy, List<QNetwork> qNetworks, List<QNetwork> targetQNetworks, Tensor.Optimizer policyOpt, List<Tensor.Optimizer> qOpts, int outputs) {
        super(m, memory);
        this.h = h;
        this.a = a;
        this.policy = policy;
...
class DDPGStrategy extends OffPolicyStrategy {
    ...
    public DDPGStrategy(HyperparamConfig h, ActionConfig a, MemoryConfig m, ReplayBuffer memory, DeterministicPolicy policy, QNetwork critic, DeterministicPolicy targetPolicy, QNetwork targetCritic, Tensor.Optimizer policyOpt, Tensor.Optimizer criticOpt, int outputs) {
        super(m, memory);
        this.h = h;
        this.policy = policy;
        this.critic = critic;
