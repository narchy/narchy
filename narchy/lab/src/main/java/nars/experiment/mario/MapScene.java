            ...
            for (int y = 0; y < 240 / 16; y++) {
                ...
                switch (level[x][y]) {
                    case TILE_LEVEL:
                        int type = data[x][y];
                        switch (type) {
                            case 0 -> g.drawImage(map[0][7], x * 16, y * 16, null);
                            case -1 -> g.drawImage(map[3][8], x * 16, y * 16, null);
                            case -3 -> g.drawImage(map[0][8], x * 16, y * 16, null);
                            case -10 -> g.drawImage(map[1][8], x * 16, y * 16, null);
                            case -11 -> g.drawImage(map[1][7], x * 16, y * 16, null);
                            case -2 -> {
                                g.drawImage(map[2][7], x * 16, y * 16 - 16, null);
                                g.drawImage(map[2][8], x * 16, y * 16, null);
                            }
                            default -> g.drawImage(map[type - 1][6], x * 16, y * 16, null);
                        }
                        break;
                    case TILE_ROAD: {
                    ...
                    case TILE_WATER:
                        for (int xx = 0; xx < 2; xx++) {
                            ...
                            for (int yy = 0; yy < 2; yy++) {
                              ...
                              var p1 = isWater(x2 + (xx), y * 2 + (yy - 1)) ? 0 : 1;
                              var p2 = isWater(x2 + (xx - 1), y * 2 + (yy)) ? 0 : 1;
                              var p3 = isWater(x2 + (xx), y * 2 + (yy)) ? 0 : 1;
...
//            }
    ...
    public void tryWalking(int xd, int yd) {
        ...
        if (level[xt][yt] == TILE_ROAD || level[xt][yt] == TILE_LEVEL) {
            ...
            xMarioA = xd * 8;
            yMarioA = yd * 8;
            moveTime = dist(x, y, xd, yd) * 2 + 1;
        }
    }

    private int dist(int x, int y, int xa, int ya) {
        int distance = 0;
        while (true) {
            x += xa;
            y += ya;
