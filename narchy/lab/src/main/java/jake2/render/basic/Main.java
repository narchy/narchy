...
public abstract class Main extends Base {
	...
	*/
	...
	private static int SignbitsForPlane(cplane_t out) {
		...
		return IntStream.range(0, 3).filter(j -> out.normal[j] < 0).map(j -> (1 << j)).reduce(0, (a, b) -> a | b);
	}
	...
	*/
	private void R_SetupGL() {
        ...
        int x = (int) Math.floor(((float)r_newrefdef.x) * vid.getWidth() / vid.getWidth());
        int x2 = (int) Math.ceil((((float)r_newrefdef.x) + r_newrefdef.width) * vid.getWidth() / vid.getWidth());
        int y = (int) Math.floor(vid.getHeight() - ((float)r_newrefdef.y) * vid.getHeight() / vid.getHeight());
        int y2 = (int) Math.ceil(vid.getHeight() - (((float)r_newrefdef.y) + r_newrefdef.height) * vid.getHeight() / vid.getHeight());
	...
	*/
	...
	private void R_Register() {
		...
		cvar_t vid_ref = Cvar.Get("vid_ref", "jogl", Defines.CVAR_ARCHIVE);
