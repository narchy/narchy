    ...
    for(i = 0; i < 10; i++) {
      ...
      for(k = 0; k < 2; k++) {
        ...
        if (k == ROAD) {
          ...
          if (lastSegment[2] < -7.25f) {
            ...
            switch (j) {
                case 0 -> slope = nextSlope;
                case 1 -> slope = -nextSlope;
                case 2 -> {
                    slope = 0;
      ...
      while(transforms.size() > 0) {
        ...
        if (hue < 0.175f) {
          ...
          switch (k) {
              case 0 -> g3.setColor(new Color(Color.HSBtoRGB(hue, 0.5f + hue, 0.7f - mag)));
              case 1 -> g3.setColor(new Color(Color.HSBtoRGB(
      ...
      for(i = 0; i < 32; i++) {
        ...
        for(j = 0; j < 256; j++) {
          ...
          if (Y < height) {
            ...
            g.setColor(backgroundColors[tx - (int) tx < 0.5f ? k : k + 1]);
