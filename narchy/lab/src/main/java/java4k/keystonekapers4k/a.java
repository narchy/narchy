        ...
        while (true) {
            ...
            do {
				...
				if (++elevatorCounter == ELEVATOR_CLOSED) {
					elevatorCounter = 0;
                    elevatorDirection = switch (elevatorFloor) {
                        case 0 -> 1;
                        case 2 -> -1;
                        default -> elevatorDirection;
                    };
				...
				for (i = queue.size() - 1; i >= 0; i--) {
					...
					switch (j) {
						case MAP_RADIO -> object[OBJ_SPRITE] = radioSprite;
						case MAP_AIRPLANE -> object[OBJ_SPRITE] = airplaneSprite;
						case MAP_BALL -> object[OBJ_Y] = object[OBJ_Y3] + (int) ballBounceY;
					}
