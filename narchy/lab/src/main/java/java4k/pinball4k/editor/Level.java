...
public class Level {
	...
	public  List<Handle> select(Rectangle rect) {
		List<Handle> list = levelObjects.stream().map(LevelObject::getHandles).flatMap(Collection::stream).filter(handle -> handle.intersects(rect)).toList();
		return list;
	}
	...
	public void write(OutputStream out, boolean writeBeziers) throws IOException {
		...
		for (LevelObject obj : levelObjects) {
            switch (obj) {
                case Flipper flipper -> flippers.add(flipper);
                case Line line1 -> lines.add(line1);
                case Sircle sircle -> sircles.add(sircle);
                case Arrow arrow -> arrows.add(arrow);
                case Bezier bezier -> {
                    if (writeBeziers) {
                        beziers.add(bezier);
                    } else {
                ...
                case null, default -> System.out.println(getClass() + " level object not supported " + obj.getClass());
            }
		}
		...
		for (int i = 0; i < groups.getSize() ; i++) {
			ArrayList<LevelObject> group = (ArrayList<LevelObject>) groups.getElementAt(i);
			dataOut.writeByte(group.size());
			int firstIdx = objIdxMap.get(group.get(0));
			if (group.size() > 0) {
				dataOut.writeByte(firstIdx);
			}
			for (LevelObject obj : group) {
				System.out.print(objIdxMap.get(obj) +" ");
			}
			System.out.print(" | ");
