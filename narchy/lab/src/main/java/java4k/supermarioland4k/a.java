              ...
              outter: while(x < 300) {
                ...
                if (x > 40 && x < 300 && random.nextInt(3) == 1) {
                  k = random.nextInt(level == 0 ? 2 : 3);
                    switch (k) {
                        case 0 -> enemies[i - 1][x + j - 5] = TYPE_CHIBIBO;
                        case 1 -> enemies[i - 2][x + j - 5] = TYPE_NOKOBON;
                        default -> enemies[i - 2][x + j - 5] = TYPE_FIGHTER_FLY;
                    }
                }
                int pipeX = 0;
          ...
          if (object[OBJ_SQUASHED] == 0) {
            ...
            outter: {
              ...
              for(x = (int)object[OBJ_X];
                ...
                k = ((x + (int)object[object[OBJ_VX] < 0 ? OBJ_X1 : OBJ_X2])) >> 3;
